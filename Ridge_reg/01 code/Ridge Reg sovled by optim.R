...
outcome <- lapply(cohort_promo_priors$landscape, function(i){
  ...
  out <- One_Ridge_Reg(tpdat_test, b_prior_test, lambda1_initial=1, lambda1_lower_bound = 1, lambda1_upper_bound = 1.1)
...
# cbind y*mean(y)
cohort_model_pred <- cohort_model_pred %>% mutate(date=as.character(date))
cohort_model_fit_add_orgYPred <- left_join(as.data.frame(cohort_model_fit), tt_mean[, c(cohort_column_name, paste0(sales_var_name,'_mean'), 'date')], by=c(cohort_column_name)) %>%
      left_join(cohort_model_pred[, c('ypred', 'cohort_id', 'date')], by=c('landscape'='cohort_id', 'date'='date')) %>%
      mutate(ypred=ypred*std_units_trev_xep_adj_mean)
# check
