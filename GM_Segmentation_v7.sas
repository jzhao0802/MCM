...
max=%foreach(v, &varlist., %nrstr(&v._max)) /*/autoname*/
;
run;
...
%end;
	...
	%end;
	proc datasets library=work nolist;
	   append base=for_large_segment data=temp2 force;
	run;
	proc datasets library=work nolist;
	append base=&out1. data=temp4 force;
	run;
	proc datasets library=work nolist;
	append base=&out2. data=temp5 force;
	run;
%end;
	data &out1.;
