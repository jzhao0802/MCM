...
select a.*, b.sum_count
from &dataset_name. a left join 
(select group, cluster, new_subgrp, sum(count) as sum_count from &dataset_name. group by group, cluster, new_subgrp) b
on a.group=b.group and a.cluster=b.cluster and a.new_subgrp=b.new_subgrp;
quit;

proc means data=temp1 noway;
  id group cluster freq new_subgrp;
  var count_sum_with_month_Besuch;
...
proc means data=temp1 nway;
  id group cluster freq new_subgrp_re count_in_subgrp;
  var count_sum_with_month_Besuch;
  output out=temp2(drop=_FREQ_ _TYPE_) mean=mean std=std min=min max=max;
...
data out.Ck_crosstb_summary_merge_revise1;
retain new_subgrp_re group cluster freq;
set out.Ck_crosstb_summary_merge_revise;
/*if new_subgrp_re=40 and count_sum_with_month_Besuch=1 then new_subgrp=2 and new_subgrp_re=41;*/
...
proc sql;
create table temp1 as
select b.specialty1_merge, b.county_text, b.group, b.cluster,  b.freq, a.new_subgrp, a.count_in_subgrp
...
select a.specialty1_merge, a.county_text, a.group, b.cluster, b.new_subgrp, a.freq as count_in_group, b.freq as count_in_cluster, b.count_in_subgrp
...
segment = _n_;
keep specialty1_merge county_text segment count mean std min max group cluster new_subgrp;
run;
...
data out.stat_table_for_clster_revise;
...
%merge_all_part(promo_data=dir_ge.for_cluster
				...
				, input2=out.stat_table_for_clster_revise
...
%macro extract_obs_for_seg(input1, input2, output);/*not complete debug add a column of original new_subgrp*/
proc sql;
select count(*) into: n from &input.;
quit;
